...
exports.newPurchase = function(req, callback) {
	var _token = typeof(req.param('security_token')) != "undefined" ? req.param('security_token') : '';
	var params = {
		where : {
			credit_hash : crypto.createHash("md5").update(req.param('credit_hash').toString()).digest("hex"),
			security_token : crypto.createHash("md5").update(req.param('credit_hash').toString()).digest("hex")
		},
	};
	console.log("here")
	kaiseki.getUsers(params, function(err, res, body, success) {
		if(typeof(body[0]) != "undefined"){
			var purchase = [ {
				...
				'purchase_type' :   parseInt(req.param('purchase_type')),
				'purchase_amount' : parseFloat(req.param('purchase_amount')),
	...
	kaiseki.getUsers(params, function(err, res, body, success) {
		if(typeof(body)!="undefined" && typeof(body[0]) != "undefined"){
			var purchase = [ {
				...
				'purchase_type'   :parseInt(req.param('purchase_type')),
				'purchase_amount' :parseFloat(req.param('purchase_amount')),
				'status'		  :body[0].liveRestrict ? config.STATUS_PURCHASE_PENDING : config.STATUS_PURCHASE_APPROVED,
			} ]
			kaiseki.createObjects(className, purchase,
					function(err, res, body, success) {
						if (err == null)
	...
	kaiseki.getUsers(params, function(err, res, body, success) {
		if(typeof(body)!="undefined" && typeof(body[0]) != "undefined"){
			var _params = {
				...
				kaiseki.getObjects(className, _params, function(err, res, body, success) {
					var i;
