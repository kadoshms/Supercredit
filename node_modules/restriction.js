		...
		if (body.length > 0) {
			params = {
				where : {
					...
					restriction_type : parseInt(req.param('purchase_type'))
				}
			};
			//is LiveRestriction feature is on
			var liveRestrict = body[0].liveRestrict;
			//user object Id
			...
			kaiseki.getObjects(className, params, function(err, res, body,success) {
				if(typeof(body[0]) != "undefined"){
					kaiseki.getObject('PurchaseTypes', body[0].restriction_purchase_type.objectId, params, function(err, res, _body, success) {
						successCallback(body[0].restriction_amount,_body.name,userId,liveRestrict)
					});
				}else
					successCallback(-1,'')
			});
